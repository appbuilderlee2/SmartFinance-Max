...
const NumPad: React.FC<NumPadProps> = ({ onNumber, onDelete, onClear, onDone, className = '' }) => {
    ...
    return (
        <div className={`grid grid-cols-4 gap-3 p-4 sf-surface border-t sf-divider ${className}`}>
            ...
            <button onClick={onDelete} aria-label="刪除一位" className={`${btnClass} text-red-400 bg-red-500/10 hover:bg-red-500/20`}><Delete size={24} /></button>
            ...
            <button onClick={onDone} aria-label="完成輸入" className={`${btnClass} row-span-2 bg-primary hover:bg-primary/90 text-white`}><Check size={32} /></button>
