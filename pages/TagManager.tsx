...
import { tagKey, normalizeTag, uniqueTags, userTags } from '../utils/tags';
...
export default function TagManager() {
  ...
  const { transactions, renameTag, deleteTag } = useData();
  ...
  const tags = useMemo(() => {
    const names = uniqueTags([...transactions.flatMap(userTags), ...loadTagHistory()]);
    return names.map(name => ({ name, count: transactions.filter(t => userTags(t).some(tag => tagKey(tag) === tagKey(name))).length })).sort((a,b) => b.count - a.count || a.name.localeCompare(b.name));
  }, [transactions, historyRevision]);
  ...
  const handleDelete = async (name: string, count: number) => {
    setDeleteError('');
    if (!await showAppConfirm(`會從 ${count} 筆交易移除「${name}」標籤，交易本身不會刪除。`, { title: '刪除標籤？', confirmLabel: '刪除標籤', destructive: true })) return;
    try {
      await deleteTag(name);
      setHistoryRevision(value => value + 1);
    } catch (error) {
      setDeleteError(error instanceof Error ? error.message : '刪除標籤失敗，請重試。');
    }
  };
