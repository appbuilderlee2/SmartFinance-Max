...
- 月曆改用單一數值月份游標，箭嘴、年份及月份選單共享同一 state，摘要每次切月都強制對應新月份。
- 摘要加入可驗證月份 key，並重新建立月份摘要節點，避免舊月份畫面殘留。
- 日期欄同時處理 `input` 及 `change` 事件，修正部分 iPhone／PWA 原生日期選擇器顯示新日期但交易仍保存舊日期。
- E2E 同時驗證箭嘴及月份選單切換後，摘要月份與金額一致。
- Service Worker 升級至 v19。

## v2.5.1 (2026-08-19)

- 修正快速切換月份時，月份箭嘴可能因舊 state 而遺失連續操作。
- 月曆、月份摘要及全月明細合併為單次交易掃描，減少切換月份時嘅同步運算。
- 摘要標題顯示實際瀏覽年份及月份，並以 live region 同步通知更新。
- 新增手機及桌面 E2E，驗證連續月份嘅摘要標題與金額即時一致。
- Service Worker 升級至 v18。

## v2.5.0 (2026-07-19)

- Apple Fluid 拆分為固定深色、固定淺色及跟隨系統三種模式。
- 跟隨系統會即時回應裝置日／夜外觀變化，毋須重新載入。
- 舊版 `applefluid` 設定自動對應至 Apple Fluid 深色，保持舊資料相容。
- 新增淺色玻璃材質、陰影、文字及對比 token；E2E 覆蓋三種模式與持久化。
- Service Worker 升級至 v17。

## v2.4.1 (2026-07-19)

- 修正切換 Apple Fluid 後立即重新載入時，非同步 IndexedDB 寫入可能被舊主題覆蓋的競態問題。
- 啟動時先協調同步主題鏡像，再更新 IndexedDB，確保主題選擇可靠保留。
...
- 新增金額私隱模式及切換 App／鎖機私隱遮罩。
- 顯示平台生物認證支援狀態，並清楚標示 PWA 目前以 PIN 解鎖。
- 設定頁加入清除瀏覽器網站資料會刪除 IndexedDB 嘅警告。
- 新安全設定沿用 IndexedDB／JSON／CSV 備份層，舊資料毋須 migration。
- 新增 PIN 衍生／驗證單元測試及手機／桌面鎖定 E2E；合共 39 個單元測試、14 個 E2E case。
- Service Worker 升級至 v14。

## v2.2.0 (2026-07-18)

- 新增常用貨幣、日期格式、每週開始日、貨幣符號位置及負數格式偏好。
- 常用貨幣套用至訂閱表單；星期起始套用至月曆；日期格式套用至提醒中心。
- 全 App 金額 formatter 支援符號前後位置及括號負數。
- 提醒中心新增預算接近上限及超支提醒，支援 70%／80%／90% 門檻。
- 新偏好沿用 IndexedDB／備份資料層，舊資料毋須 migration。
- 單元測試增至 38 個，E2E 擴充第二階段設定持久化驗證。
- Service Worker 升級至 v13。

## v2.1.0 (2026-07-18)

- 設定頁升級為可搜尋嘅設定與資料管理中心，並保留原有帳務及個人化入口。
- 顯示 IndexedDB 後端、主要資料數量、資料項目數量及估算儲存空間。
- 新增資料完整性檢查；JSON／CSV 還原支援合併或取代及內容摘要。
- 還原前自動匯出現況復原備份；危險刪除必須輸入「刪除」確認。
- 加入檢查更新、固定重新載入及清快取但保留財務資料操作。
- 新增設定資料中心及快取資料安全 E2E；手機／桌面合共 12 個 case。
- Service Worker 升級至 v12。
