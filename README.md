...
## v2.15.0 對話框、帳單儲存及資料安全

- 確認、輸入及提示視窗統一為 iOS 風格 App 對話框，支援鍵盤操作及安全區。
- 信用卡帳單等金額按貨幣精度輸入；只有資料成功寫入後畫面才會更新。
- 跨分頁寫入加上版本檢查，舊分頁會先停止修改，避免覆蓋新資料。
- 設定頁清楚說明 PIN 只限制畫面存取，財務資料未有加密。
- 保留全部六個底部入口。

## v2.9.0 信用卡獨立幣別及到期提示

- 每張信用卡獨立設定幣別；年費、信用額度及信用卡中心按卡片幣別顯示。
- 新帳單自動沿用卡片幣別；升級時為舊卡片及舊帳單固定當時使用的主幣別，往後修改卡片幣別不會改寫歷史帳單。
- 待繳總額按幣別分開計算，並分別顯示逾期、七日內即將到期及未輸入金額數量。
- 帳單可同時顯示多個警示，例如「逾期」及「未輸入金額」。
...
- 設定首頁收納為六個分類，點入顯示詳細選項；搜尋及分類保留於網址，返回及重新載入可恢復。
- 其他主題、CSV、資料庫檢查及進階刪除預設摺疊，搜尋相關項目時展開。
- 訂閱服務由統計總覽頂部獨立進入；設定移除訂閱及信用卡重複入口。
- 保留原有備份、還原、私隱及刪除確認流程；記帳方法不變。

## v2.8.0 信用卡中心

- 底部新增「信用卡」，整合總覽、週期及卡片管理；設定頁提供統一入口。
- 待繳金額按幣別分開，未輸入金額另行提示；可按繳款月份及狀態查看帳單。
- 卡片詳情提供金額輸入、已繳標記及取消、帳單紀錄、下一期及卡片設定。
- 沿用既有卡片／週期資料；標記繳款只更新記錄，不會實際扣款。
- 驗證限制：本機預覽瀏覽器被阻擋，手機視覺及新流程實際操作尚未驗證；依使用者要求先合併。

## v2.7.1 返回報告及儲存提示

- 查看交易後返回報告，保留期間、幣別、篩選、分類及已載入明細位置
- 移除常駐「已儲存」浮動提示；儲存中及失敗時仍會提示

## v2.7.0 報告與標籤新介面

- 報告分為總覽、分類及趨勢；月份箭嘴切換、收支走勢及分類比例使用真實帳目計算
- 點分類查看交易明細，再進入交易詳情及編輯；清楚顯示期間及幣別
- 篩選改為底部面板，套用才更新；備註文字、標籤、分類及金額範圍分開處理
- 無資料只顯示一個空狀態，提供上月及全部期間入口
- 新增／編輯帳目共用標籤選擇面板，支援搜尋、最近使用、新增及移除已選標籤
- 設定新增標籤管理，可查看交易數量及標籤報告；重新命名或合併前預覽受影響交易
- 訂閱來源以系統欄位識別，新訂閱不再自動加入 subscription 手動標籤；舊資料保留
- 標籤合併保留備註、金額及分類，並去除同筆交易的重複標籤
