...
## v2.6.0 可靠性、逐筆儲存及效能更新

- IndexedDB schema 升級至 v2，舊交易陣列會以單一原子 transaction 遷移成獨立交易記錄，並加入日期及分類索引
- 一般新增、修改或刪除只寫入有變更嘅交易；5,000 筆交易測試中，修改一筆只執行一次交易 row `put`
- 儲存狀態以 IndexedDB commit 結果為準；寫入失敗會顯示錯誤、保留待寫入工作，成功重試後自動清除提示
- 刪除單次週期交易會保存略過日期，避免下次開啟 App 時自動補回；亦可停止未來週期並保留歷史記錄
- 備份匯入會檢查欄位、重複 ID、分類關聯及版本；合併模式按 ID 合併，避免覆蓋整個資料集合
- 跨日、返回前景及重新取得焦點時，會刷新訂閱、週期交易及預算狀態
- 離線版本使用 build ID 及 SHA-256 驗證全部資產；下載不完整或損壞時不會取代可用版本
- 月曆以月份索引讀取交易；記錄頁延後搜尋並每次漸進顯示 100 筆，減少大量資料時嘅畫面運算
- 主題及信用卡狀態與唯讀帳目狀態分離，避免無關設定更新觸發大量帳目元件重新渲染
- CI 已通過 55 項單元／回歸測試、16 項手機尺寸及桌面 Chromium E2E，以及 TypeScript production build

### 升級前後注意事項

- 建議升級前先於設定頁匯出 JSON 備份
- 首次開啟 v2.6.0 會自動遷移舊資料；如果舊版分頁阻擋資料庫升級，請關閉其他 SmartFinance-Max 分頁後重試
- 舊版程式不能直接讀取已升級嘅 IndexedDB v2；如需回復舊版，應使用升級前匯出嘅 JSON 備份
- 收據仍附於個別交易，啟動 App 及匯出備份時仍會載入全部交易

## v2.5.2 月曆修正

- 月曆箭嘴、年份及月份選單統一使用同一月份狀態，摘要會即時對應畫面顯示月份
- 修正部分 iPhone／PWA 日期選擇器顯示新日期、交易卻保存舊日期嘅問題

## v2.5.1 月曆效能修正

- 修正快速連續切換月份時可能遺失操作嘅問題
- 月曆、月份摘要及全月明細共用單次交易掃描，降低切換月份時嘅同步運算
...
- 修正選擇主題後立即重新載入時，IndexedDB 非同步寫入可能尚未完成的競態問題
- 以同步主題鏡像協調 IndexedDB，確保 Apple Fluid 設定於重新載入後仍然保留
